...
- **문제**: 시/도 단위 지역 조회 시 `LIKE '11%'` 조건을 사용하면서 대량의 데이터를 스캔하는 문제가 발생했습니다.
- **원인**: 복합 인덱스 `idx_rent_query`의 선두 컬럼인 `region_code`가 `LIKE '11%'`로 처리되어, 이후 컬럼들이 인덱스 탐색에 활용되지 못했었습니다.
- **해결**: 시군구 코드 25개를 사전에 전개하여 `LIKE` 조건을 `IN (...)` 조건으로 변경했습니다. 각 지역 코드가 동등 조건으로 처리되도록 변경하여 복합 인덱스를 보다 효율적으로 활용하도록 개선했습니다.
- **결과**:
  - 실행 시간: 658ms → 357ms (1.8배 개선)
  - 스캔 행 수: 187만 → 42만 (4.4배 감소)
  - 조회 결과의 정확성을 유지하면서 쿼리 성능 개선
 
### 🔴 PriceModel Redis 캐시 배치 조회 최적화

- **문제**: 가격 모델 캐시의 40개 조합을 개별 조회하고 있어, 각 추천 알고리즘 실행 시 동일한 조회가 반복되면서 요청당 최대 80회의 Redis 왕복이 발생했습니다.
- **원인**: 캐시 조회·저장 로직이 단건 처리 방식으로 구현되어 있어 여러 키를 조회할 때마다 Redis와 개별 통신이 발생하고 있었습니다.
- **해결**: 40개 캐시 키를 `Redis MGET` 한 번으로 일괄 조회하도록 변경했습니다. 또한 새롭게 계산된 캐시 데이터는 saveAll()을 통해 Redis Pipeline으로 일괄 저장하도록 개선했습니다.
- **결과**: 
  - Redis 왕복 횟수: 80회 → 2회
  - 소요 시간: 32.2ms → 1.35ms
  - 처리 시간 약 24배 개선
